...
function calculator(string) {
    ...
    if (letter.length > 0)
        throw Error('Символы не допустиы, Вы ввели это :' + letter);
    let vars = string.split(/[+\-*\/g]/)
    if (vars.length != 2)
        throw Error('долэно быть 2 операнда')
    const isRome = /^[IVXLCDMZ]+$/i;

    const r = vars.reduce((s, v) => s + isRome.test(v), 0);
    if (r === 1)
        throw Error('Оба числа должны быть арабскими или римскими цифрами');
    else if (r === 2)
        vars = vars.map(v => rom2arab(v));
    if (vars.some(v => v < 1 || v > 10))
        throw Error('Допустимое значение опернадов от 1 до 10 включительно');
